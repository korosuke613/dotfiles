...
- `~/dotfiles/.gitignore`
- `~/dotfiles/mac/setup.sh`（asdfリンク削除、office.gitconfig対応）
- `~/dotfiles/mac/zsh/.zshrc`（source先のファイル名変更対応）
...
# Before → After
~/.rd/bin → $HOME/.rd/bin
~/.config/op/plugins.sh → $HOME/.config/op/plugins.sh
~/.bun/_bun → $HOME/.bun/_bun
~/.local/bin → $HOME/.local/bin
~/.turso → $HOME/.turso
~/.lmstudio/bin → $HOME/.lmstudio/bin
```
