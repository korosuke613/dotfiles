# dotfiles-sync.sh (retired)

このスクリプトは廃止予定です。`.zshrc` からは呼び出されません。
シェル起動時のブランチ切り替え、rebase、staging、commit、pushを
再び有効化しないでください。後継の明示的な `dot sync` コマンドへ
移行するまで、通常のGitコマンドを使って内容を確認して同期します。

dotfiles リポジトリを `sync` ブランチで自動同期していたスクリプトです。
