...
- `sync` ブランチが日常の作業ベース。新しいブランチは `sync` から分岐させること
- `main` ブランチはリリース用。main から分岐してはならない
- `sync` と `main` ではファイル構成が異なる場合があり、main ベースで作業するとコンフリクトする
...
- `cp` は `cp -i` にエイリアスされている。Bashツールで上書きコピーする場合は `/bin/cp` を使うこと
